...
class MidiFile(mido.MidiFile):
        ...
        for idx, channel in enumerate(events):
            ...
            for msg in channel:
                ...
                if msg.type == "note_off":
                    ...
                    if type(note_register[msg.note]) != int:
                        note_on_end_time = note_register[msg.note][0]
                        intensity = note_register[msg.note][1]
                    # else:

                    #     print(note_register[msg.note])
                    #     intensity = note_register[msg.note]
                    #     note_on_end_time = note_register[msg.note]
    ...
    def draw_roll(self):
        ...
        plt.ylim(32,80)
        plt.xlim(0,10*x_label_interval)
...
if __name__ == "__main__":
    ...
    # mid.draw_roll()
    print("loading model")
    model = InterpolatorMusicModel(dim_per_head=64,num_heads=10,num_layers=12,vocab_sz=3242+1,device="cpu")
    model.load_state_dict(torch.load("12_layerd/musicformer.pt"))
    embs = model.genre_embeddings.weight.cpu().detach().numpy()
    embs = normalize(embs,axis=1)
    corr = embs@embs.T
    print(corr.shape)
    fig, ax = plt.subplots()
    im = plt.imshow(corr, cmap='afmhot',axes=ax)
    with open("genre_converter.json","rb") as f:
        labels_dict = json.load(f)
    labels_dict = dict([(value, key) for key, value in labels_dict.items()])
    ax.set_xticks(np.arange(len(labels_dict)))
    ax.set_xticklabels([labels_dict[i] for i in range(len(labels_dict))], rotation=90)

    # Set the y-tick labels
    ax.set_yticks(np.arange(len(labels_dict)))
    ax.set_yticklabels([labels_dict[i] for i in range(len(labels_dict))])
    ...
    sns.set_theme()
    a = np.load("12_layerd/losses.npy")
    fig,axes = plt.subplots(1,2)
    print(a)
    plt.sca(axes[0])
    plt.plot(a[:,0],label="training")
    plt.plot(a[:,1],label="validation")
    plt.sca(axes[1])
    plt.loglog(a[:,0],label="training")
    plt.loglog(a[:,1],label="validation")
